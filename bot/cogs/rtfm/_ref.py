...
async def _process_mozilla_doc(ctx: Context[Parrot], url: str):
...
async def _git_main_ref(part: str, ctx: Context[Parrot], text: str):
...
async def sql_ref(ctx: Context[Parrot], text: str):
    ...
    for tag in tuple(intro.next_siblings):
        assert tag is not None and isinstance(tag, bs4.Tag)
...
async def haskell_ref(ctx: Context[Parrot], text: str):
