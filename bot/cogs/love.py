...
class Love(commands.Cog):
    ...
    def __init__(self, bot: Parrot) -> None:
        ...
        love_matches = self.bot.assets.love_matches
        self.love_data = sorted((int(key), value) for key, value in love_matches.items())
    ...
    def load_comp_json(self):
        ...
        return self.bot.assets.zodiac_compatibility, self.bot.assets.zodiac_explanation
    ...
    async def pickupline(self, ctx: Context) -> None:
        ...
        """
        random_line = random.choice(self.bot.assets.pickup_lines["lines"])
        embed = discord.Embed(title=":cheese: Your pickup line :cheese:", description=random_line["line"], color=ctx.author.color)
        embed.set_thumbnail(url=random_line.get("image", self.bot.assets.pickup_lines["placeholder"]))
        await ctx.send(embed=embed)

    @commands.command()
    async def myvalenstate(self, ctx: Context, *, name: str | None = None) -> None:
        ...
        for state in self.bot.assets.valenstates.keys():
