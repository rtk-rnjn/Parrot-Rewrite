...
if TYPE_CHECKING:
    ...
    from ..context import Context, Parrot
...
class ShortTime:
    ...
    def __init__(self, argument: str, *, now: datetime.datetime | None = None, tzinfo: datetime.tzinfo = datetime.timezone.utc):
        ...
        if match is None or not match.group(0):
            ...
            if match is not None:
                ...
                self.delta = self.dt - (now or datetime.datetime.now(datetime.timezone.utc))
                if tzinfo is not datetime.timezone.utc:
                    self.dt = self.dt.astimezone(tzinfo)
                    self.delta = self.dt - (now or datetime.datetime.now(tzinfo))
        ...
        self.delta = self.dt - now
        if tzinfo is not datetime.timezone.utc:
            self.dt = self.dt.astimezone(tzinfo)
            self.delta = self.dt - now.astimezone(tzinfo)

    @classmethod
    async def convert(cls, ctx: Context[Parrot], argument: str) -> Self:
...
class RelativeDelta(app_commands.Transformer, commands.Converter):
    ...
    def __do_conversion(cls, argument: str) -> relativedelta:
        ...
        data = {k: int(v) for k, v in match.groupdict(default=0).items()}
        return relativedelta(**data)  # pyright: ignore[reportArgumentType]

    async def convert(self, _: Context[Parrot], argument: str) -> relativedelta:
        try:
            return self.__do_conversion(argument)
        except ValueError as e:
            raise commands.BadArgument(str(e)) from None

    async def transform(self, interaction: discord.Interaction[Parrot], value: str) -> relativedelta:
        try:
...
class HumanTime:
    ...
    def __init__(self, argument: str, *, now: datetime.datetime | None = None, tzinfo: datetime.tzinfo = datetime.timezone.utc):
        ...
        assert isinstance(status, pdt.pdtContext) and isinstance(dt, datetime.datetime)
...
class TimeTransformer(app_commands.Transformer):
    async def transform(self, interaction: discord.Interaction[Parrot], value: str) -> datetime.datetime:
        tzinfo = datetime.timezone.utc
        tzinfo = await interaction.client.get_tzinfo(interaction.user.id)
...
class FriendlyTimeResult:
    ...
    async def ensure_constraints(self, ctx: Context[Parrot], uft: UserFriendlyTime, now: datetime.datetime, remaining: str) -> None:
        if self.dt < now:
            raise commands.BadArgument("This time is in the past.")
...
class UserFriendlyTime(commands.Converter):
    ...
    async def convert(self, ctx: Context[Parrot], argument: str) -> FriendlyTimeResult:
